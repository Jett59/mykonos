...
Hpet::Hpet(void* physicalAddress) : registerPointer((uint64_t*)memory::mapAddress(physicalAddress, 1024, false)) {
  ...
  writeRegister(HPET_REGISTER_GENERAL_CONFIGURATION,
                readRegister(HPET_REGISTER_GENERAL_CONFIGURATION) & ~HPET_CONFIGURATION_LEGACY_BIT);
  reset();
}
Hpet::~Hpet() {
...
void Hpet::reset() {
  // Stop the counter
  writeRegister(HPET_REGISTER_GENERAL_CONFIGURATION,
                readRegister(HPET_REGISTER_GENERAL_CONFIGURATION) & ~HPET_CONFIGURATION_ENABLE_BIT);
  // Set it to 0
