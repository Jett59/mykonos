...
void writePixel(unsigned x, unsigned y, Pixel pixel) {
  if (x < getWidth() && y < getHeight()) {
    unsigned pixelOffset = (y * frameBuffer.pitch + x * (frameBuffer.depth / 8));
    ...
    memcpy((void*)displayPixelPointer, (void*)pixelPointer, frameBuffer.depth / 8);
  }
}
...
void writeBitmap(unsigned initialX, unsigned initialY, unsigned width, unsigned height, void* bitmap, Pixel foreground, Pixel background) {
  ...
  for (unsigned y = 0; y < height; y++) {
    ...
    unsigned pixelOffset = ((y + initialY) * frameBuffer.pitch + initialX * bytesPerPixel);
...
void copyRegion(unsigned x, unsigned y, unsigned width, unsigned height, unsigned newX, unsigned newY) {
  ...
  for (unsigned yOffset = 0; yOffset < height; yOffset++) {
    unsigned sourceOffset = ((y + yOffset) * frameBuffer.pitch + x * bytesPerPixel);
    void* sourcePointer = (void*)((uint8_t*)backBuffer + sourceOffset);
    unsigned destOffset = ((newY + yOffset) * frameBuffer.pitch + newX * bytesPerPixel);
