...
extern "C" void* memset(void* str, int c, size_t n) {
  void* tmpStr = str;
  __asm__ volatile("rep stosq" : "+D"(tmpStr) : "a"((unsigned char)c * 0x0101010101010101), "c"(n / 8) : "memory");
  __asm__ volatile("rep stosb" : : "D"(tmpStr), "c"(n & 7), "a"((unsigned char)c) : "memory");
  return str;
}
extern "C" void* memcpy(void* dst, const void* src, size_t n) {
  void* tmpDst = dst;
  const void* tmpSrc = src;
  __asm__ volatile("rep movsq" : "+D"(tmpDst), "+S"(tmpSrc) : "c"(n / 8) : "memory");
  __asm__ volatile("rep movsb" : : "D"(tmpDst), "S"(tmpSrc), "c"(n & 7) : "memory");
  return dst;
}
